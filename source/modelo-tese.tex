% Exemplo de dissertacao do INF-UFG com texto em portugues formatado com LaTeX
\documentclass[relatorio]{inf-ufg}
% OpÁıes da classe inf-ufg (ao usar mais de uma, separe por vi≠rgulas)
%   [tese]         -> Tese de doutorado.
%   [dissertacao]  -> DissertaÁ„o de mestrado (padr„o).
%   [monografia]   -> Monografia de especializaÁ„o.
%   [relatorio]    -> RelatÛrio final de graduaÁ„o.
%   [abnt]         -> Usa o estilo "abnt-alf" de citaÁ„o bibliogr·fica.
%   [nocolorlinks] -> Os links de navegaÁ„o no texto ficam na cor preta.
%                     Use esta opÁ„o para gerar o arquivo para impress„o
%                     da vers„o final do seu texto!!!

\usepackage{placeins}
%\usepackage[utf8]{inputenc}


%----------------------------------------------------- INICIO DO DOCUMENTO %
\begin{document}

%------------------------------------------ AUTOR, TÕTULO E DATA DE DEFESA %
\autor{Ana LetÌcia Herculano da Silva}
\autorR{Silva, Ana LetÌcia Herculano da}

\titulo{TÈcnicas e CritÈrios de Testes em uma AplicaÁ„o de Banco de Dados Relacioanal}
\subtitulo{Estudo de Caso}

\cidade{Goi‚nia}
\dia{03} %
\mes{07} % Data da apresentaÁ„o/defesa do trabalho
\ano{2017} % Formato numÈrico: \dia{01}, \mes{01} e \ano{2009}

%-------------------------------------------------------------- ORIENTADOR %
\orientador{C·ssio Leonardo Rodrigues}
\orientadorR{Rodrigues, C·ssio Leonardo}


\coorientador{Dr. Edmundo SÈrgio Spoto}
\coorientadorR{Dr. Spoto, Edmundo SÈrgio}

%-------------------------------------------------- INSTITUI«√O E PROGRAMA %
\universidade{Universidade Federal de Goi·s}
\uni{UFG}
\unidade{Instituto de Inform·tica}


\universidadeco{ Universidade Federal de Goi·s}
\unico{UFG}
\unidadeco{Instituto de Inform·tica}

\programa{Sistemas de InformaÁ„o}
\concentracao{Teste de Software}

%-------------------------------------------------- ELEMENTOS PR…-TEXTUAIS %
\capa    % Gera o modelo da capa externa do trabalho
\publica % Gera a autorizaÁ„o para publicacao em formato eletronico
\rosto   % Primeira folha interna do trabalho

\input{./pre/pre_aprovacao}
\input{./pre/pre_direitos}
\input{./pre/pre_dedicatoria}
\input{./pre/pre_agradecimentos}
\input{./pre/pre_epigrafe}
\input{./pre/pre_resumo}
\input{./pre/pre_abstract}

\tabelas[figtab]
%OpÁıes:
%nada [] -> Gera apenas o sum·rio
%fig     -> Gera o sum·rio e a lista de figuras
%tab     -> Sum·rio e lista de tabelas
%alg     -> Sum·rio e lista de algoritmos
%cod     -> Sum·rio e lista de cÛdigos de programas
%
% Pode-se usar qualquer combinaÁ„o dessas opÁıes.
% Por exemplo:
%  figtab       -> Sum·rio e listas de figuras e tabelas
%  figtabcod    -> Sum·rio e listas de figuras, tabelas e
%                  cÛdigos de programas
%  figtabalg    -> Sum·rio e listas de figuras, tabelas e algoritmos
%  figtabalgcod -> Sum·rio e listas de figuras, tabelas, algoritmos e
%                  cÛdigos de programas

%--------------------------------------------------------------- CAP√çTULOS %
\input{./tex/cap_I}
\input{./tex/cap_II}
\input{./tex/cap_III}
\input{./tex/cap_IV}
\input{./tex/cap_V}
\input{./tex/cap_VI}
\input{./tex/cap_VII}
\input{./tex/cap_VIII}

%------------------------------------------------------------ BIBLIOGRAFIA %
\cleardoublepage
\nocite{*} %%% Retire esta linha para gerar a bibliografia com apenas as
           %%% referÍncias usadas no seu texto!
\arial
\bibliography{./bib/modelo-tese} %%% Nomes dos seus arquivos .bib
\label{ref-bib}

%--------------------------------------------------------------- AP NDICES %
\apendices

\input{./pos/apend_I}
\input{./pos/apend_II}
\input{./pos/apend_III}
\input{./pos/apend_IV}


\end{document}

%------------------------------------------------------------------------- %
%        F I M   D O  A R Q U I V O :  m o d e l o - t e s e . t e x       %
%------------------------------------------------------------------------- %
